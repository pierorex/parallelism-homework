...
int main( int argc, char *argv[] ) {
	...
	// barrier:  every process waits here until all of them get here too. (this is called sync)
	MPI_Barrier(MPI_COMM_WORLD);
	MPE_Start_log();
	for (j = 0; j < 5; j++) {
		MPE_Log_event(event1a, 0, "start broadcast");
		// process 0 sends to every other 1 integer. A process != 0 will receive in this same instruction
		...
		// every process sends its part 'mypi' to be reduced into 'pi' inside process '0'
