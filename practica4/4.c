...
main (int argc, char **argv) {
	...
	MPI_Type_vector(IMAX, 1, JMAX-1, MPI_INT, &coltype);
	MPI_Type_commit(&coltype);

	if ( myrank == SOURCERANK ) {
		/* El proceso SOURCERANK reparte la matriz usando el nuevo tipo de dato
		derivado definido */ 
		MPI_Send(&matrix[0][JMAX-1], 1, coltype, 1, 0, MPI_COMM_WORLD);
	}
	else {
		/* el resto de los procesos recibe la porción de la matriz que le
		corresponde */
		MPI_Request request;
		MPI_Irecv(&matrix[0][JMAX-1], 1, coltype, SOURCERANK, 0, MPI_COMM_WORLD, &request);
